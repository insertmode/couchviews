...
/**
 * Load all design documents and callback `cb(designDocs)`.
 *
 * @param {Function} cb
 */
...
Db.prototype.loadDesignDocs = function (cb) {
  ...
  db.request('GET', this.url + query, null, function (err, result) {
    if (err) { throw new Error('error when getting design docs: ' + err); }
    ...
    var designDocs = [], 
        ...
        step = function () {
          current = current + 1;
          if (current >= result.rows.length) { return cb(designDocs); }
        };
    
    result.rows.forEach(function (row) {
      that.loadDesignDoc(row.id, function (designDoc) {
...
/**
 * Load design document with `id` and callback `cb(designDoc)`.
 *
 * @param {String} id
 * @param {Function} cb
 */
...
Db.prototype.saveDesignDocs = function (designDocs, cb) {
  if (designDocs.length === 0) { return cb(); }

  var that = this,
  saved = 0,
  step = function () {
    saved = saved + 1;
    if (saved >= designDocs.length) { return cb(); }
  };
...
Db.prototype.saveDesignDoc = function (designDoc, cb) {
  ...
  db.save(designDoc, function (err) {
    if (err) { throw new Error('error when saving design document: ' + err); }
    return cb();
  });
};
